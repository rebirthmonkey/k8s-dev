...
# 3. rewrite the history on that branch to *only* include staging/src/10_sample-apiserver
...
# this command rewrite git history to *only* include staging/src/10_sample-apiserver
git filter-branch -f --subdirectory-filter staging/src/10_sample-apiserver HEAD
