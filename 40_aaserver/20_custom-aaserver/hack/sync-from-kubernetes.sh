...
# 3. rewrite the history on that branch to *only* include staging/src/20_custom-aaserver
...
# this command rewrite git history to *only* include staging/src/20_custom-aaserver
git filter-branch -f --subdirectory-filter staging/src/20_custom-aaserver HEAD
